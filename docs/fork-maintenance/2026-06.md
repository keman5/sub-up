...
| 2026-06-26 | 前端多处仍直接使用浏览器原生 `window.confirm` / `window.prompt` 或裸 `confirm(...)`，在后台高风险操作、账号错误码提醒、备份恢复密码输入等场景会弹出浏览器默认确认框，和 fork 的 `BaseDialog` / `ConfirmDialog` 视觉与焦点管理不一致，也容易在后续同步中继续漏出 native dialog。 | 新增 `frontend/src/composables/useAppDialog.ts` 和 `frontend/src/components/common/AppDialogHost.vue`，在 `frontend/src/App.vue` 全局挂载 promise 化弹窗宿主，复用项目 `BaseDialog` 样式；将备份恢复/删除、账号批量删除/重置/刷新、创建/编辑/批量编辑账号 429/529 自定义错误码提醒、用户平台限额清空/重置、风控 flagged hash 清空、运维系统日志回滚/清理、设置页 Web Search 用量重置/Admin API Key 重新生成或删除、邮件模板恢复官方模板等调用改为 `await appDialog.confirm(...)`；`window.prompt` 改为 `appDialog.askText(...)`，包括 2026-07-29 上游新增的 `frontend/src/components/user/profile/ProfilePasskeyCard.vue` Passkey 重命名流程。新增 `frontend/src/components/common/__tests__/nativeDialogUsage.spec.ts` 扫描前端非测试源码，防止再次引入 `alert` / `confirm` / `prompt` 原生调用。 | `./node_modules/.bin/vitest run src/components/common/__tests__/nativeDialogUsage.spec.ts src/components/payment/__tests__/PaymentProviderDialog.nativeAlert.spec.ts`；`./node_modules/.bin/vue-tsc --noEmit`；`./node_modules/.bin/vite build`；`git diff --check`；`rg --pcre2 -n "\\bwindow\\.(?:alert|confirm|prompt)\\b|(?<![\\w.])(?:alert|confirm|prompt)\\s*\\(" frontend/src -S --glob '!node_modules' --glob '!dist' --glob '!**/__tests__/**'` 无命中。 | 同步官方后搜索 `window.confirm`、`window.prompt`、`window.alert`、`confirm(`、`ProfilePasskeyCard`、`AppDialogHost`、`useAppDialog`、`nativeDialogUsage.spec.ts`。如果官方引入自己的全局 dialog service，可以收敛到官方实现，但必须继续满足：前端非测试源码不得出现浏览器原生 alert/confirm/prompt；备份恢复仍能通过项目弹窗输入密码；高风险批量操作和清理类操作仍需等待异步确认后再执行。 |
...
| 2026-06-27 | 公告系统只有站内展示和弹窗/静默通知，管理员无法在发布重要公告时同步通过邮件触达全部用户或指定用户。 | 新增公告邮件推送能力：公告创建/更新请求支持 `email_push_mode=none|all|selected` 和 `email_push_user_ids`；服务层保存公告后按显式选择发送 `announcement.publish` 邮件，`all` 只取 active 用户，`selected` 过滤重复、非 active、无邮箱和不存在用户；编辑公告表单默认“不推送邮件”，避免误群发；邮件模板编辑器新增公告邮件推送事件说明。2026-07-29 上游扩展 `UserRepository` 后，公告服务改依赖仅含 `GetByID` / `ListWithFilters` 的 `AnnouncementUserRepository`，避免无关余额写接口破坏公告模块编译。 | `cd backend && go test ./internal/service ./internal/handler/admin -run 'TestAnnouncement|TestNotificationEmail' -count=1`；`/opt/homebrew/bin/pnpm --dir frontend run typecheck`；`/opt/homebrew/bin/pnpm --dir frontend run build`；`git diff --check`。 | 同步官方后搜索 `announcement.publish`、`AnnouncementEmailPushMode`、`AnnouncementUserRepository`、`email_push_mode`、`email_push_user_ids`。如果官方新增公告通知能力，可收敛到官方模型，但必须保留“显式一次性推送、不因公告到达 starts_at 或用户打开公告自动补发、编辑默认不推送”的行为；公告用户仓储只依赖实际读取方法，不要重新扩大为完整 `UserRepository`。 |
