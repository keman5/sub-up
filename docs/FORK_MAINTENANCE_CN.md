...
- Claude/Anthropic 示例从 OpenAI/Codex base 派生时，绝对 URL 必须取 `origin` 作为 Claude base，避免 Claude Code 在已有路径后再拼 `/v1/messages`。
- 现有线上副本示例：
...
- Claude 配置中的 `ANTHROPIC_BASE_URL` 使用 `buildClaudeBaseUrl()`；绝对 URL 形式的 OpenAI/Codex base 应生成 `https://<host>`。
...
| 2026-06-04 | 订阅管理批量分配弹窗的“最近注册用户”和搜索结果只显示邮箱与 ID，运营无法快速用用户名或备注辨认用户。 | 扩展管理员用量搜索用户接口 `/admin/usage/search-users`，在简化用户结果中返回 `username` 和 `notes`；前端 `SimpleUser` 类型、最近注册用户映射和 `SubscriptionsView.vue` 用户选择列表同步展示“用户名 · 备注”，已选用户标签也显示相同副信息。 | `go test ./internal/handler/admin -run TestUsageSearchUsersIncludesDisplayFields`；`pnpm --dir frontend exec vitest run src/api/__tests__/admin.usage.spec.ts`；`pnpm --dir frontend run typecheck`；`pnpm --dir frontend exec eslint src/views/admin/SubscriptionsView.vue src/api/admin/usage.ts src/api/__tests__/admin.usage.spec.ts`。 | 同步官方后搜索 `SearchUsers`、`SimpleUser`、`formatSimpleUserMeta` 和 `toSimpleUser`。如果官方重构订阅分配弹窗或用户搜索接口，必须继续确保最近注册用户与搜索结果都能显示用户名或管理员备注。 |
...
| 2026-06-04 | 账号状态倒计时偶发显示 `common.time.countdown.daysHours` 等翻译 key，说明倒计时格式化在语言包未就绪或旧包缺少 key 时会把 key 原样暴露到界面。 | `frontend/src/utils/format.ts` 为 `formatCountdown()` 和 `formatCountdownWithSuffix()` 增加 `translateOrFallback()` 兜底：i18n 返回原 key 时改用内置短格式 `Xd Yh`、`Xh Ym`、`Xm` 和 `{time} to lift`；新增 `formatCountdown.spec.ts` 覆盖缺失消息时不会显示 key。 | `pnpm --dir frontend exec vitest run src/utils/__tests__/formatCountdown.spec.ts`；`pnpm --dir frontend exec eslint src/utils/format.ts src/utils/__tests__/formatCountdown.spec.ts`；`pnpm --dir frontend run typecheck`。 | 同步官方后搜索 `formatCountdown`、`formatCountdownWithSuffix` 和 `translateOrFallback`。如果官方重构时间格式化，必须继续保证倒计时缺失翻译时不会把 `common.time.countdown.*` key 暴露给用户。 |
