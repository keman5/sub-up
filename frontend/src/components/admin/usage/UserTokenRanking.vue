      ...
      <table class="w-full min-w-max divide-y divide-gray-200 dark:divide-dark-700">
        ...
        <tbody class="divide-y divide-gray-200 bg-white dark:divide-dark-700 dark:bg-dark-900">
          ...
          >
            ...
            <td
              class="max-w-[260px] px-4 py-3 text-sm font-medium text-gray-700 dark:text-gray-200"
              :title="[item.email, item.notes].filter(Boolean).join('\n')"
            >
              <div class="truncate">
                {{ item.email || `User #${item.user_id}` }}
                <span class="ml-1 font-normal text-gray-400 dark:text-gray-500">#{{ item.user_id }}</span>
              </div>
              <div v-if="item.notes?.trim()" class="mt-0.5 truncate text-xs font-normal text-gray-400 dark:text-gray-500">
                {{ item.notes.trim() }}
              </div>
            </td>
