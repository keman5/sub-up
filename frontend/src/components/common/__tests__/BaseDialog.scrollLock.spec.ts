...
describe('dialog body scroll lock', () => {
  ...
  it('keeps the lock until the last visible dialog closes', async () => {
    ...
    expect(locked()).toBe(true)
    ;(second.vm as unknown as { handleAfterLeave: () => void }).handleAfterLeave()
    expect(locked()).toBe(true)
    await first.setProps({ show: false })
    expect(locked()).toBe(true)
    ;(first.vm as unknown as { handleAfterLeave: () => void }).handleAfterLeave()
  ...
  it('releases the lock after repeated open and close cycles', async () => {
    ...
    expect(locked()).toBe(true)
    ;(wrapper.vm as unknown as { handleAfterLeave: () => void }).handleAfterLeave()
    expect(locked()).toBe(false)
    await wrapper.setProps({ show: true })
    wrapper.unmount()
    expect(locked()).toBe(false)
