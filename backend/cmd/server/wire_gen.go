...
func initializeApplication(buildInfo handler.BuildInfo) (*Application, error) {
	...
	announcementService := service.ProvideAnnouncementService(announcementRepository, announcementReadRepository, userRepository, userSubscriptionRepository, notificationEmailService)
	...
	openAIGatewayHandler := handler.NewOpenAIGatewayHandler(openAIGatewayService, concurrencyService, billingCacheService, subscriptionService, apiKeyService, usageRecordWorkerPool, errorPassthroughService, contentModerationService, opsService, configConfig)
	handlerSettingHandler := handler.ProvideSettingHandler(settingService, buildInfo, notificationEmailService)
	totpHandler := handler.NewTotpHandler(totpService)
	handlerPaymentHandler := handler.NewPaymentHandler(paymentService, paymentConfigService, channelService)
	paymentWebhookHandler := handler.NewPaymentWebhookHandler(paymentService, registry)
	availableChannelHandler := handler.NewAvailableChannelHandler(channelService, apiKeyService, settingService)
	openAITroubleshootingAIClient := service.NewOpenAITroubleshootingAIClient(accountRepository, openAIGatewayService)
	redisTroubleshootingRateLimiter := service.ProvideTroubleshootingRateLimiter(redisClient)
	systemTroubleshootingEvidenceProvider := service.NewSystemTroubleshootingEvidenceProvider(accountRepository, opsRepository)
	troubleshootingAssistantService := service.ProvideTroubleshootingAssistantService(openAITroubleshootingAIClient, redisTroubleshootingRateLimiter, systemTroubleshootingEvidenceProvider, settingRepository, notificationEmailService)
	troubleshootingHandler := handler.NewTroubleshootingHandler(troubleshootingAssistantService)
	idempotencyCoordinator := service.ProvideIdempotencyCoordinator(idempotencyRepository, configConfig)
	idempotencyCleanupService := service.ProvideIdempotencyCleanupService(idempotencyRepository, configConfig)
	handlers := handler.ProvideHandlers(authHandler, userHandler, apiKeyHandler, usageHandler, redeemHandler, subscriptionHandler, announcementHandler, channelMonitorUserHandler, adminHandlers, gatewayHandler, openAIGatewayHandler, handlerSettingHandler, totpHandler, handlerPaymentHandler, paymentWebhookHandler, availableChannelHandler, troubleshootingHandler, idempotencyCoordinator, idempotencyCleanupService)
	...
	openAIQuotaRefreshService := service.ProvideOpenAIQuotaRefreshService(accountRepository, accountUsageService)
	proxyExpiryService := service.ProvideProxyExpiryService(proxyRepository)
	subscriptionExpiryService := service.ProvideSubscriptionExpiryService(userSubscriptionRepository, settingRepository, notificationEmailService, leaderLockCache, db)
	accountPoolHealthNotifyService := service.ProvideAccountPoolHealthNotifyService(accountRepository, settingRepository, notificationEmailService)
	scheduledTestRunnerService := service.ProvideScheduledTestRunnerService(scheduledTestPlanRepository, scheduledTestService, accountTestService, rateLimitService, configConfig)
	paymentOrderExpiryService := service.ProvidePaymentOrderExpiryService(paymentService, leaderLockCache, db)
	channelMonitorRunner := service.ProvideChannelMonitorRunner(channelMonitorService, settingService)
	userPlatformQuotaUsageFlusher := service.ProvideUserPlatformQuotaUsageFlusher(configConfig, billingCache, serviceUserPlatformQuotaRepository, timingWheelService)
	v := provideCleanup(client, redisClient, opsMetricsCollector, opsAggregationService, opsAlertEvaluatorService, opsCleanupService, opsScheduledReportService, opsSystemLogSink, schedulerSnapshotService, tokenRefreshService, accountExpiryService, openAIQuotaRefreshService, proxyExpiryService, subscriptionExpiryService, accountPoolHealthNotifyService, usageCleanupService, idempotencyCleanupService, pricingService, emailQueueService, billingCacheService, usageRecordWorkerPool, subscriptionService, oAuthService, openAIOAuthService, geminiOAuthService, antigravityOAuthService, grokOAuthService, openAIGatewayService, scheduledTestRunnerService, backupService, paymentOrderExpiryService, channelMonitorRunner, userPlatformQuotaUsageFlusher)
