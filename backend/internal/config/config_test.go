...
func TestLoadHeadroomStatsFromEnv(t *testing.T) {
	resetViperWithJWTSecret(t)
	t.Setenv("HEADROOM_STATS_ENABLED", "true")
	t.Setenv("HEADROOM_STATS_URL", "http://headroom-a2:8787/stats")
	t.Setenv("HEADROOM_STATS_TIMEOUT_SECONDS", "5")

	cfg, err := Load()
	require.NoError(t, err)
	require.True(t, cfg.HeadroomStats.Enabled)
	require.Equal(t, "http://headroom-a2:8787/stats", cfg.HeadroomStats.URL)
	require.Equal(t, 5, cfg.HeadroomStats.TimeoutSeconds)
}

func TestValidateOpenAIOAuthCodexResponsesURL(t *testing.T) {
	resetViperWithJWTSecret(t)
...
func TestValidateHeadroomStatsConfig(t *testing.T) {
	...
	cfg.HeadroomStats.Enabled = true
	cfg.HeadroomStats.URL = "http://headroom-a2:8787/stats"
	cfg.HeadroomStats.TimeoutSeconds = 2
	require.NoError(t, cfg.Validate())

	cfg.HeadroomStats.URL = "/stats"
	require.ErrorContains(t, cfg.Validate(), "headroom_stats.url invalid")

	cfg.HeadroomStats.URL = "http://headroom-a2:8787/stats"
	cfg.HeadroomStats.TimeoutSeconds = 0
	require.ErrorContains(t, cfg.Validate(), "headroom_stats.timeout_seconds")
}
