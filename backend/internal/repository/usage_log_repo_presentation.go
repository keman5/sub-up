...
func (r *usageLogRepository) getEndpointStatsByColumnWithFiltersForView(ctx context.Context, endpointColumn string, startTime, endTime time.Time, userID, apiKeyID, accountID, groupID int64, model string, modelSource string, requestType *int16, stream *bool, billingType *int8, billingMode string, usePresentation bool) (results []EndpointStat, err error) {
	presentationFactor := usagePresentationFactorSQL("", usePresentation)
	totalTokensExpr := usagePresentationTotalTokensSQL("", presentationFactor)
	totalCostExpr := usagePresentationCostSQL("total_cost", presentationFactor)
	actualCostExpr := usagePresentationCostSQL("actual_cost", presentationFactor)
	if accountID > 0 && userID == 0 && apiKeyID == 0 {
		actualCostExpr = usagePresentationCostSQL("COALESCE(account_stats_cost, total_cost) * COALESCE(account_rate_multiplier, 1)", presentationFactor)
	}
...
func (r *usageLogRepository) getEndpointPathStatsWithFiltersForView(ctx context.Context, startTime, endTime time.Time, userID, apiKeyID, accountID, groupID int64, model string, modelSource string, requestType *int16, stream *bool, billingType *int8, billingMode string, usePresentation bool) (results []EndpointStat, err error) {
	presentationFactor := usagePresentationFactorSQL("", usePresentation)
	totalTokensExpr := usagePresentationTotalTokensSQL("", presentationFactor)
	totalCostExpr := usagePresentationCostSQL("total_cost", presentationFactor)
	actualCostExpr := usagePresentationCostSQL("actual_cost", presentationFactor)
