...
func TestSettingHandler_UpdateSettings_RoundTripsOpenAIHeadroomEnabled(t *testing.T) {
	gin.SetMode(gin.TestMode)
	repo := &settingHandlerRepoStub{
		values: map[string]string{
			service.SettingKeyPromoCodeEnabled: "true",
	...
	require.Equal(t, http.StatusOK, rec.Code)
	require.Equal(t, "true", repo.values[service.SettingKeyOpenAIHeadroomEnabled])
	...
	data, ok := resp.Data.(map[string]any)
	require.True(t, ok)
	require.Equal(t, true, data["openai_headroom_enabled"])
}

func TestSettingHandler_GetSettings_ReturnsPersistedOpsMonitoringEnabledWithoutOpsService(t *testing.T) {
	gin.SetMode(gin.TestMode)
	repo := &settingHandlerRepoStub{
		values: map[string]string{
			service.SettingKeyPromoCodeEnabled:     "true",
	...
	data, ok := resp.Data.(map[string]any)
	require.True(t, ok)
	require.Equal(t, true, data["ops_monitoring_enabled"])
}
