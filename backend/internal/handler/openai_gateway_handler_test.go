...
func TestOpenAIHandleStreamingAwareError_JSONEscaping(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			assert.Equal(t, service.ClientErrorMessageForAcceptLanguage("", tt.message), errorObj["message"])
		})
	}
}
...
func TestOpenAIEnsureForwardErrorResponse_ImageJSONKeepaliveWritesSingleJSONFallback(t *testing.T) {
	...
	require.Equal(t, "Upstream request failed (上游请求失败)", gjson.Get(w.Body.String(), "error.message").String())
}
...
func TestOpenAIResponses_APIKeyPassthroughPool5xxRetriesThenExhaustsMaxSwitches(t *testing.T) {
	...
	require.Equal(t, "Upstream service temporarily unavailable (上游服务暂时不可用)", gjson.GetBytes(rec.Body.Bytes(), "error.message").String())
}
...
func TestOpenAIResponses_APIKeyPassthroughSSERateLimitUsesConfiguredPoolRetry(t *testing.T) {
	...
	require.Equal(t, service.ClientErrorMessageForAcceptLanguage("", "Upstream rate limit exceeded, please retry later"), gjson.GetBytes(rec.Body.Bytes(), "error.message").String())
}
